...
Servo myservo[8];
const int servo_pin[8]={6,5,8,9,10,11,12,13};
...
//p为停止，j为校准。k为运动(k:前进;b:后退;l:左转;r:右转）,w为校准状态。
char token="p";
//舵机校准角度
char jz[8]={0,0,0,0,0,0,0,0};
...
char balance[8]= {30,30,30,30,30,30,30,30};
...
void read_servo_jz()
{
  for(int i=0;i<8;i++)
  {
    jz[i]=EEPROM.read(servo_jz+i);
    Serial.print("jz[");Serial.print(i);Serial.print("]=");Serial.print(int(jz[i]));Serial.print(";");
  }
    Serial.println(); 
    Serial.println("read servo_jz_data successfull!");
    
}
//站立姿势
void zhanli()
{
        myservo[0].write(90+(int(balance[0]))*direct[0]+jz[0]);
        myservo[1].write(90+(int(balance[4]))*direct[1]+jz[1]);
        myservo[2].write(90+(int(balance[4]))*direct[1]+jz[2]);
        myservo[3].write(90+(int(balance[0]))*direct[0]+jz[3]);
    
    
    
        myservo[4].write(90+direct[4]*(int(balance[0]))+jz[4]);
        myservo[5].write(90+direct[5]*(int(balance[4]))+jz[5]);
        myservo[6].write(90+direct[5]*(int(balance[4]))+jz[6]);
        myservo[7].write(90+direct[4]*(int(balance[0]))+jz[7]);
}
//校准姿势。
void jiaozhunzhanli()
{
...
void loop() {
  ...
  if(token=='j')//校准舵机角度
  {
    while(Serial.available()>0)
    {
      ...
      jz[servo_id]=Serial.read();
      jz[servo_id]=Serial.parseInt();
      EEPROM.write(servo_jz+servo_id,jz[servo_id]);
  
      for(int j=0;j<8;j++)
        myservo[j].write(90+jz[j]);
  
      read_servo_jz();//每次校准一个角度之后打印出来一个角度
    }
    token='w';
  }
  else if(token=='k')
  {
      for(int i=0;i<43;i++)
      {
        myservo[0].write(90+(int(wk[i][0]))*direct[0]+direct[0]*(jz[0]));
        myservo[1].write(90+(int(wk[i][1]))*direct[1]+direct[1]*(jz[1]));
        myservo[2].write(90+(int(wk[i][0]))*direct[2]+direct[2]*(jz[2]));
        myservo[3].write(90+(int(wk[i][1]))*direct[3]+direct[3]*(jz[3]));
    
    
    
        myservo[4].write(90+direct[4]*(int(wk[i][4]))+direct[4]*(jz[4]));
        myservo[5].write(90+direct[5]*(int(wk[i][5]))+direct[5]*(jz[5]));
        myservo[6].write(90+direct[6]*(int(wk[i][4]))+direct[6]*(jz[6]));
        myservo[7].write(90+direct[4]*(int(wk[i][5]))+direct[7]*(jz[7]));

        
        
        //Serial.println(int(wk[i][4]));
        delay(servo_speed);
      }
  }
  else if(token=='b')
  {
    for(int i=0;i<45;i++)
      {
        myservo[0].write(90+(int(bk[i][0]))*direct[0]+direct[0]*(jz[0]));
        myservo[1].write(90+(int(bk[i][1]))*direct[1]+direct[1]*(jz[1]));
        myservo[2].write(90+(int(bk[i][0]))*direct[2]+direct[2]*(jz[2]));
        myservo[3].write(90+(int(bk[i][1]))*direct[3]+direct[3]*(jz[3]));
    
    
    
        myservo[4].write(90+direct[4]*(int(bk[i][4]))+direct[4]*(jz[4]));
        myservo[5].write(90+direct[5]*(int(bk[i][5]))+direct[5]*(jz[5]));
        myservo[6].write(90+direct[6]*(int(bk[i][4]))+direct[6]*(jz[6]));
        myservo[7].write(90+direct[4]*(int(bk[i][5]))+direct[7]*(jz[7]));
        
        //Serial.println(int(bk[i][4]));
        delay(servo_speed);
      }
  }
  else if(token=='l')
  {
    for(int i=0;i<43;i++)
      {
        myservo[0].write(90+(int(wkL[i][0]))*direct[0]+direct[0]*(jz[0]));
        myservo[1].write(90+(int(wkL[i][1]))*direct[1]+direct[1]*(jz[1]));
        myservo[2].write(90+(int(wkL[i][0]))*direct[2]+direct[2]*(jz[2]));
        myservo[3].write(90+(int(wkL[i][1]))*direct[3]+direct[3]*(jz[3]));
    
    
    
        myservo[4].write(90+direct[4]*(int(wkL[i][4]))+direct[4]*(jz[4]));
        myservo[5].write(90+direct[5]*(int(wkL[i][5]))+direct[5]*(jz[5]));
        myservo[6].write(90+direct[6]*(int(wkL[i][4]))+direct[6]*(jz[6]));
        myservo[7].write(90+direct[4]*(int(wkL[i][5]))+direct[7]*(jz[7]));
  ...
  else if(token=='r')
  {
    for(int i=0;i<43;i++)
      {
        myservo[0].write(90+(int(wkR[i][0]))*direct[0]+direct[0]*(jz[0]));
        myservo[1].write(90+(int(wkR[i][1]))*direct[1]+direct[1]*(jz[1]));
        myservo[2].write(90+(int(wkR[i][0]))*direct[2]+direct[2]*(jz[2]));
        myservo[3].write(90+(int(wkR[i][1]))*direct[3]+direct[3]*(jz[3]));
    
    
    
        myservo[4].write(90+direct[4]*(int(wkR[i][4]))+direct[4]*(jz[4]));
        myservo[5].write(90+direct[5]*(int(wkR[i][5]))+direct[5]*(jz[5]));
        myservo[6].write(90+direct[6]*(int(wkR[i][4]))+direct[6]*(jz[6]));
        myservo[7].write(90+direct[4]*(int(wkR[i][5]))+direct[7]*(jz[7]));
