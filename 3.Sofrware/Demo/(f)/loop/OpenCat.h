...
class Motion {
  public:
    ...
    void loadDataFromProgmem(unsigned int pgmAddress) {
      ...
      for (int i = 0; i < 2; i++)//读取预期roll、和pitch角
...
//通过片上eeprom地址加载里面的数据，数据存储的时候先存储了技能的类别，然后下一个字节开始为技能的数据地址
    void loadDataByOnboardEepromAddress(int onBoardEepromAddress) {
      ...
      //如果技能类型为本能，则从I2CE2Prom里加载数据（根据数据指针地址-在dataArrayAddress）
    ...
    void loadBySkillName(char* skillName){//调用lookupAddressByName根据名字找到在eeprom里存储的地址（指向技能类型）
      ...
      if (onBoardEepromAddress == -1)//（没找到输入的命令）
...
float adjust(byte i) {
  ...
  if (i == 1 || i > 3)  {//check idx = 1
    ...
    if ((leftQ && RollPitchDeviation[0]*slope  > 0 )|| ( !leftQ && RollPitchDeviation[0]*slope  < 0))
  ...
  return ((i > 3 ? postureOrWalkingFactor : 1) *rollAdj + RollPitchDeviation[1] * adaptiveCoefficient(i, 1) );
}
//该函数用于存储舵机校准校准角度数据
