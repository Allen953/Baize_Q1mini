...
void beep(int8_t note, float duration = 10, int pause = 0, byte repeat = 1 ) {
  if (note == 0) {//空拍，不会响，仅仅起到延时作用
    analogWrite(BUZZER, 0);
    delay(duration);
    return;
  }
  int freq = 220 * pow(1.059463, note - 1); // 1.059463 comes from https://en.wikipedia.org/wiki/Twelfth_root_of_two
  float period = 1000000.0 / freq;          //一个脉冲周期（微秒）
  for (byte r = 0; r < repeat; r++) {       //repeat为当前音符重复发声次数（拍数）
    for (float t = 0; t < duration * 1000; t+= period){//duration*1000为音符时长（微秒）
      analogWrite(BUZZER, 150);      //任何除0和255外的数都行，150发出的音最好听
