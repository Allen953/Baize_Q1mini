...
class Motion {
  public:
    ...
    int lookupAddressByName(char* skillName){//通过技能名字寻找地址
      ...
      for (byte s = 0; s < NUM_SKILLS; s++) {//在片上e2prom里存储技能信息
        ...
        delete[]readName;
        skillAddressShift += 3;//1字节类型，1整型地址，一共3字节
      }
      PTLF("wrong key!");
    ...
    void loadDataFromProgmem(unsigned int pgmAddress) {
      period = pgm_read_byte(pgmAddress);//读取技能帧数
      for (int i = 0; i < 2; i++)//读取预期roll、和pitch角并转换成弧度
        expectedRollPitch[i] = radPerDeg * (int8_t)pgm_read_byte(pgmAddress + 1 + i);
      byte frameSize = period > 1 ? WALKING_DOF : 16;//判断每帧大小，帧数大于1，则每帧WALKING_DOF=8歌数据，帧数为1，则为16个数据
      int len = period * frameSize;//计算总数据个数：总舵机数据个数=帧数*每帧数据个数
      dutyAngles = new char[len];//新建dutyAngles数组存储舵机运动数据
      for (int k = 0; k < len; k++) {//从pgm里面读取舵机运动数据存入dutyAngles数组里面
    ...
    void loadDataFromI2cEeprom(unsigned int &eeAddress){
      ...
      for (int i = 0; i < 2; i++)
        expectedRollPitch[i] = radPerDeg * (int8_t)Wire.read();//读取预期roll、和pitch角并转换成弧度
      ...
      int readFromEE = 0;
      int readToWire = 0;
      while (len > 0) {//单次让iic eeprom芯片传输数据量，每次最多只能获取WIRE_BUFFER=30个数据
        ...
        } while (--len > 0 && ++readToWire<WIRE_BUFFER);//每次读取到WIRE_BUFFER限制的时候，重新开启下一页。
      }//读数据结束
    }
...
//通过板载eeprom地址加载里面的数据，数据存储的时候先存储了技能的类别，然后下一个字节开始为技能的数据地址
    void loadDataByOnboardEepromAddress(int onBoardEepromAddress) {
      ...
      unsigned int dataArrayAddress = EEPROMReadInt(onBoardEepromAddress + 1);//读取技能数据地址
      delete[] dutyAngles;//释放储存原命令舵机运动数据的内存
#ifdef DEVELOPER
      PTF("free memory: ");
      ...
      if (skillType == 'I') { //从板载I2C EEPROM拷贝本能数据指针 
    ...
    //通过技能名字寻找数据地址
    void loadBySkillName(char* skillName) {//调用lookupAddressByName根据名字找到在eeprom里存储的地址（指向技能类型）
      int onBoardEepromAddress = lookupAddressByName(skillName);
      if (onBoardEepromAddress == -1)
        return;     //如果找到地址，则根据这个地址调用loadDataByOnboardEepromAddress去加载数据
...
//在片上eeprom上分配技能地址（新技），saveSkillInfoFromProgmemToOnboardEeprom()
//函数将技能信息存储到了片上eeprom，同时把本能的数据存储到了I2C EEPROM并在片上EE
//PROM分配了数据在I2C EEPROM的地址，然而新技的信息除数据地址之外也存储到了片上EE
//PROM，但是数据地址还未在片上EEPROM分配，这个函数主要负责对新技数据地址进行EEPROM的存储
...
//这个函数的作用是让小喵从当前的Posture移动到目标的Posture，offset为起始关节号，pos为0，gait为8
void transform(char * target,  float speedRatio=1, byte offset=0){
  char *diff = new char[DOF - offset], maxDiff = 0;//定义diff数组存储其余关节的角度差
  for (byte i = offset; i < DOF; i++) {//计算其余关节角度差存入diff数组
    diff[i - offset] =   currentAng[i] - target[i - offset];
    maxDiff = max(maxDiff, abs( diff[i - offset]));//计算maxDiff为其余动作关节的最大运动幅度（角度），即diff数组里的最大值
  }
  //默认速度是一步一度，如果speedRatio为2，则相比默认两步并一步，相当于将从当前posture运动到目标
  //posture的工程氛围steps个步骤，即一共分为steps帧
  byte steps = byte(round(maxDiff /1.0/ speedRatio));
  for (byte s = 0; s <= steps; s++)//循环这steps帧
    for (byte i = offset; i < DOF; i++){//控制每帧运动，控制每个关节运动到这帧的具体数据
      float dutyAng = (target[i - offset] + (steps == 0 ? 0 : (1 + cos(M_PI * s / steps)) / 2 * diff[i - offset]));
      calibratedPWM(i,  dutyAng);//控制i号关节运动到对应角度
...
void behavior(int n, char** skill, float *speedRatio, int *pause){
  for (byte i = 0; i < n; i++) {
    motion.loadBySkillName(skill[i]);//加载技能（pos）列表里第i个技能的数据
    transform( motion.dutyAngles, speedRatio[i]);//控制小猫做第i个姿势
    delay(pause[i]);
  }
}
...
//从EEPROM的EEaddress地址开始读取len个字节数据并从串口打印
